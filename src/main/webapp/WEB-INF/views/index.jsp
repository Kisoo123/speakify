...
<div id="root-div">
    ...
    <div id="header-main-div">
        ...
        <div id="main-div">
            ...
            <p><c:out value="${loginMember != 'anonymousUser'}"> 안녕하세요</c:out></p>
...
<c:if test="${loginMember!='anonymousUser'}"> //로그인 후 실행되는 스크립트
    ...
    <script>
        <%--                    알람관련                --%>
            ...
            // 음성채팅 관련
            let socket = new WebSocket('ws://'+window.location.host+'/signal');
            ...
            function initializeWebSocket() {
                if (socket.readyState === WebSocket.CLOSED || socket.readyState === WebSocket.CLOSING) {
                    ...
                    socket = new WebSocket('ws://'+window.location.host+'/signal');
                }
            }
                ...
                success: function (response) {
                    ...
                    if (response.length > 0) {
                        response.forEach(alarm => {
                            let alarmElement = `
                        <div class="list-group-item list-group-item-action py-2 lh-tight bg-dark">
                            <div class="d-flex w-100 align-items-center justify-content-between">
                                <strong class="mb-1">\${alarm.type}</strong>
                                <small class="text-muted">\${new Date(alarm.createAt).toLocaleString()}</small>
                            </div>
                            <small class="text-muted">알림 ID: \${alarm.id}, 사용자 ID: \${alarm.toUser}</small>
                        </div>
                    `;
